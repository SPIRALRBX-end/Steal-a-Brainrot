...
local TARGET_POSITION = Vector3.new(-410.752, -9.25, 78.906)
local DRAG_SPEED = 0.3
local DRAG_ACTIVE = false
local dragConnection = nil
local bodyVelocity = nil
local lastPosition = nil
local movementTracker = nil
...
local function updatePositionHistory(position)
	table.insert(positionHistory, {pos = position, time = tick()})
	if #positionHistory > maxHistorySize then
		table.remove(positionHistory, 1)
	end
end

local function getSmoothedVelocity(currentPos, targetPos)
	...
	local baseSpeed = math.min(distance * 2, 30)
	local smoothedSpeed = baseSpeed * smoothDelta
	
	return direction * smoothedSpeed
end
...
local function startDrag()
	...
	movementTracker = RunService.Heartbeat:Connect(function()
		if not DRAG_ACTIVE or not bodyVelocity or not humanoidRootPart then return end
		
		local currentPos = humanoidRootPart.Position
		updatePositionHistory(currentPos)
		
		local newVelocity = getSmoothedVelocity(currentPos, TARGET_POSITION)
	...
	dragConnection = RunService.Heartbeat:Connect(function()
		...
		local currentPos = humanoidRootPart.Position
		local distance = (TARGET_POSITION - currentPos).Magnitude
...
local function formatNameWithRarity(name, rarity)
	...
	if nameLength < maxNameLength then
		local spacesNeeded = maxNameLength - nameLength
		padding = string.rep(" ", spacesNeeded)
	end
	
	return name .. padding .. " - " .. rarityText
end
...
local function populateScrolling()
	...
	-- Cria botões para cada animal
	for index, name in ipairs(availableNames) do
		...
		btn.Name = name
		-- CORREÇÃO: Posiciona cada botão em Y diferente
		btn.Position = UDim2.new(0, 0, 0, currentY)
		btn.Size = UDim2.new(0.973592997, 0, 0, buttonHeight) -- Altura fixa em pixels
		...
		btn.Font = Enum.Font.SourceCodePro
		...
		local formattedText = formatNameWithRarity(name, rarity)

		-- Define aparência baseada na seleção
	...
	for i = 1, 3 do
		...
		zzz.Name = "zzz" .. i
		zzz.Position = UDim2.new(0, 0, 0, currentY)
		zzz.Size = UDim2.new(0.973593, 0, 0, buttonHeight)
		zzz.Parent = scrollingFR
		
		currentY = currentY + buttonHeight + buttonSpacing
	end

	scrollingFR.CanvasSize = UDim2.new(0, 0, 0, currentY)
end
