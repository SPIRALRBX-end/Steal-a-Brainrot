...
local function getSmoothedMovement(currentPos, targetPos)
	...
	local speed = math.min(distance * 0.8, DRAG_SPEED)
...
local function startDrag()
	...
	local currentDistance = (TARGET_POSITION - humanoidRootPart.Position).Magnitude
	
	if currentDistance > teleportThreshold then
		humanoidRootPart.CFrame = CFrame.new(TARGET_POSITION + Vector3.new(0, 10, 0))
		wait(0.1)
	end
	...
	movementTracker = RunService.Heartbeat:Connect(function()
		if not DRAG_ACTIVE or not bodyPosition or not bodyVelocity or not humanoidRootPart then return end
		
		local currentPos = humanoidRootPart.Position
		updatePositionHistory(currentPos)
		
		local targetDirection = getSmoothedMovement(currentPos, TARGET_POSITION)
		local newTargetPos = currentPos + (targetDirection * smoothDelta)
		
		bodyPosition.Position = newTargetPos
		bodyVelocity.Velocity = targetDirection * 0.5
	...
	dragConnection = RunService.Heartbeat:Connect(function()
		if not DRAG_ACTIVE or not bodyPosition then return end
